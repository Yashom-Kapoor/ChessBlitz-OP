...
export default function LessonBubble({ name, description, offset, available }: LessonBubbleProps) {
    ...
    return (
        <View style={{...bubbleStyles.bubble, transform: `translate(${offset.x - (isTablet ? 250 : 150)}px, ${offset.y - (isTablet ? 100 : 75)}px)`}}>
            ...
            <TouchableOpacity style={{...bubbleStyles.button, opacity: available ? 1 : 0.3}} onPress={available ? onPress : (() => {})}>
                {available ? (
                    <Image resizeMode='contain' style={{...bubbleStyles.play}} source={require('@/assets/images/play.png')}/>
                ) : (
                    <Image resizeMode='contain' style={{...bubbleStyles.play}} source={require('@/assets/images/lock.png')}/>
                )}
            </TouchableOpacity>
        </View>
    )
